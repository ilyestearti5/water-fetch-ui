...
import { mergeObject, tw, useCopyState, useEffectDelay } from "@/functions/react-utils";
import { useKeyboard } from "@/reducers/Global/keyboard.slice";
...
export const KeyboardView = () => {
  ...
  const shiftGlobalState = useKeyboard("Shift");
  const forceShiftGlobalState = useCopyState(false);
  const controlGlobalState = useKeyboard("Control");
  const altGlobalState = useKeyboard("AltGraph");
  React.useEffect(() => {
  ...
  const clickedPosition = useCopyState<pos>([-100, -100]);
  ...
  const position = useCopyState<pos>([0, innerHeight - 300]);
  return (
    <div
